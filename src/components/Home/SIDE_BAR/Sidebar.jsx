import React, {useState} from 'react';
import { Link } from 'react-router-dom';
import Logo from '../../Logo/Logo';
import { Collections, HouseboatTwoTone, Person, Settings, ArrowLeft } from '@mui/icons-material';
import './sideBar.css';

const Sidebar = () => {
  const [show, setShow] = useState(false);
  return (
    <div className={`side-bar ${show ? 'min-bar' :''}`}>
        <Logo/>
        <div className="lists">
            <button>
              <Link className='link' to={`../home`}>
                <span>
                  <HouseboatTwoTone className="bi"/>
                </span>
                <span>Home</span>
              </Link>
            </button>
            <button>
              <Link className='link' to={`../profile`}>
                <span>
                  <Person className="bi"/>
                </span>
                <span>Profile</span>
              </Link>
            </button>
            <button>
              <Link className='link' to={`../your-post`}>
                <span>
                  <Collections className="bi"/>
                </span>
                <span>Your Posts</span>
              </Link>
            </button>
            <button>
              <Link className='link' to={`../settings`}>
                <span>
                  <Settings className="bi"/>
                </span>
                <span>Settings</span>
              </Link>
            </button>
            <button className='logOut'>
              <Link to={`/`} className="link">
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-box-arrow-down-left" viewBox="0 0 16 16">
                <path fillRule="evenodd" d="M7.364 12.5a.5.5 0 0 0 .5.5H14.5a1.5 1.5 0 0 0 1.5-1.5v-10A1.5 1.5 0 0 0 14.5 0h-10A1.5 1.5 0 0 0 3 1.5v6.636a.5.5 0 1 0 1 0V1.5a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 .5.5v10a.5.5 0 0 1-.5.5H7.864a.5.5 0 0 0-.5.5z"/>
                <path fillRule="evenodd" d="M0 15.5a.5.5 0 0 0 .5.5h5a.5.5 0 0 0 0-1H1.707l8.147-8.146a.5.5 0 0 0-.708-.708L1 14.293V10.5a.5.5 0 0 0-1 0v5z"/>
              </svg>
              </span>
              <span>Log Out</span>
              </Link>
            </button>
        </div>
        <div className="toggleSL" onClick={() => {show? setShow(false): setShow(true)}}>
          <ArrowLeft className={`icon-large ${show ? 'rotate-180' :'rotate-0'}`}/>
        </div>
    </div>
  )
}

export default Sidebar;