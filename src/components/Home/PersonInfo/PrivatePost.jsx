...
const Posts = ({ kit }) => {
  const { image, message, comment_count, likes, userID } = kit;
  ...
  const phpdata = async () => {
    ...
    formdata.append("messageId", kit.id);
    formdata.append("comments_", comments_.replace(/'/g, "\\'"));
    console.log(comments_, "POSTEDID::", kit.id)
      ...
      <div className={`msg_holder message_and_likes ${display ? "show" : "hide"}`}>
        ...
        <div className="messages">
          ...
          <div className="messages_comment comments_">
            {
              allmsg !== undefined || allmsg.length > 0 ? allmsg.map((i, k) => i.messageId === kit.id ? <Comment input={i} key={k} /> : '') : ''
            }
          </div>
        </div>
      </div>
